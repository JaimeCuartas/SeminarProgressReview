...
\textbf{Context-Free Grammars and Pushdown Automata}

To provide a foundation for the proposed explanation
extraction methods, 
standard definitions and notations
for Context-Free Grammar and Pushdown Automata are adopted
...
%
  ...
  \begin{itemize}
    %
    \item $\gamma =\gamma_1 v(v\in V)$
    %
    \footnote{$\gamma_1 \in V^*$ represents the remaining symbols on the
    ...
    %
    , $x=ax' (a\in \Sigma)$,
    $\gamma'=\gamma_1 m(m\in V*)$ and $(q', m)\in \delta(q, a, v)$, namely ``a-move'';
    %
    \item or $\gamma =\gamma_1 v(v\in V)$, $x=x'$, $\gamma'=\gamma_1m(m\in V^*)$
  ...
  %
  And the \emph{language} recognized by a PDA
  is the set of all accepted words.
  %
\end{definition}

A language is context-free iff there exists
a PDA $\mathcal{A}$ 
that accepts the same language (L(G)=L(M)).
Thus, provide an explanation for
a PDA decision or its respective grammar is
equivalent.

%
The following CFG that generates the
...
% \begin{equation}
  % \begin{aligned}
  %     \text{Balanced} \to& \text{Nested Balanced} \\
  %     \text{Balanced} \to& \text{Unclosed Closed} \\
  %     \text{Balanced} \to&\text{Pair Balanced} \\
  %     \text{Balanced} \to& \text{Open Close} \\
  %     \text{Nested} \to& \text{Unclosed Close} \\
  %     \text{Unclosed} \to& \text{Open Balanced} \\
  %     \text{Pair} \to& \text{Open Close} \\
  %     \text{Open} \to& \text{'('} \\
  %     \text{Close} \to& \text{')'}
  % \end{aligned}
  % \end{equation}
...
\begin{figure}[t]
  \centering
    \input{/Users/jcua0001/Documents/Monash-Thesis-Template-Latex-main/Figures/PDA/parenthesis_PDA.tex}
...
It is a well-known result that for every PDA M,
there exists an equivalent CFG G that generates
the same language $L(M)=L(G)$.
This equivalence allows us to utilize grammar-based
parsing algorithms, such as CYK, to analyze the
behavior of stack-based automata.

\begin{equation}
\begin{aligned}
    \text{Balanced} &\to \text{Open Suffix} \\
    \text{Suffix} &\to \text{Balanced Suffix (Nested content)} \\
    \text{Suffix} &\to \text{Close Balanced (Close current, start new)} \\
    \text{Suffix} &\to \text{')' (End block)} \\
    \text{Open} &\to \text{'('} \\
    \text{Close} &\to \text{')'}
\end{aligned}
\end{equation}
