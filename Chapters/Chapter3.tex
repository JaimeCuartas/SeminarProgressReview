...
extraction methods, I establish the formal definitions
...
\begin{definition}[Pushdown Automaton]
  ...
  \begin{itemize}
    ...
    %
    \item $\Sigma$ is the input alphabet, equivalent to
    the terminals in a Context-Free Grammar.
    %
    \item $\Gamma$ is the stack alphabet. A finite set
    of symbols that can be pushed onto or popped from
    the stack.
    %
    \item $\delta:Q\times(\Sigma\cup\{\epsilon\})\times \Gamma \rightarrow Q \times \Gamma^*$
    \item is the transition fuction. 
    It dictates how the machine transitions between states and modifies the stack based on the current state, input symbol, and the top symbol of the stack.
    %
