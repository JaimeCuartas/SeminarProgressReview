...
Here, we adopted standard definitions and notations for \emph{finite
...
explanations (CXps).
%
Informally, an AXp answers ``why does $\mathcal{A}$ accept/reject
$w$?''
%
a CXp answers ``How can $w$ be modified to alter the response
of~$\mathcal{A}$?''.
%
...
\begin{figure}[b]
  \centering
    ...
    \caption{
      Input \texttt{bbbbb} is accepted by this automata.
...
\begin{example}\label{ex:bbbbb}
  ...
  where
  the underlined characters are those that explain the acceptance.
  %
  But other (shorter) explanations exist, e.g.
  ...
  Note that once we have one type of explanation, we can easily
  extract the other type of explanation by Minimal Hitting sets
  as it is illustrated in \autoref{fig:mhs_explanations_dot}.
  Note that AXps fix indices while CXps free them and reveals
  an minimal set where it is possible to flip the prediction
  (get a rejected word).
  %
  \qed
  %
...
languages of explanations are found in Appendix.