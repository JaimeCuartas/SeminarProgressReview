...
%
\begin{itemize}
    \item $\mathcal{CXP}_1 = \{1, 2\}$: Generates \texttt{\underline{((}))}.
    \item $\mathcal{CXP}_2 = \{1, 3\}$: Generates \texttt{\underline{(})\underline{(})}.
\end{itemize}
%
...
\begin{itemize}
  %
  \item $\mathcal{AXP}_1 = \{1\}$: The first symbol `\texttt{)}'
  guarantees rejection regardless of the remaining suffix.
  %
  \item $\mathcal{AXP}_2 = \{2, 3\}$: The substring `\texttt{))}'
...
\begin{table}[h]
  ...
  \end{tabular}
  \caption{
    Modified CYK Table verifying $\cxp(\{1,2\}, \texttt{))))})$.
    %
